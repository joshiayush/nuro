 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "\n",
    "1. **Descriptive statistics** — summarizing and visualizing data (mean, variance, correlation)\n",
    "2. **Inferential statistics** — drawing conclusions from data and fitting models (MLE, information theory)\n",
    "\n",
    "### Prerequisites\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "1. **Where is the center** of the data? (Measures of central tendency)\n",
    "2. **How spread out** is the data? (Measures of spread)"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "In NumPy:\n",
    "- `np.var(data, ddof=0)` — population variance (divides by $N$)\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "|:---:|:---|\n",
    "| $\\rho = 1$ | Perfect positive linear relationship — points lie exactly on an upward line |\n",
    "| $\\rho = -1$ | Perfect negative linear relationship — points lie exactly on a downward line |\n",
    "| $\\rho = 0$ | No *linear* relationship (but there might be a non-linear one!) |\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "1. **Diagonal entries** = variance of each feature ($\\mathrm{Cov}(X_i, X_i) = \\mathrm{Var}(X_i)$)\n",
    "2. **Off-diagonal entries** = covariance between pairs of features\n",
    "3. **The matrix is always symmetric**: $\\mathrm{Cov}(X, Y) = \\mathrm{Cov}(Y, X)$\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "$$f(x_i \\mid \\mu, \\sigma^2) = \\dfrac{1}{\\sqrt{2\\pi\\sigma^2}} \\exp\\left(-\\dfrac{(x_i - \\mu)^2}{2\\sigma^2}\\right)$$\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "1. **Non-negative:** $D_{KL}(p \\| q) \\geq 0$ — always\n",
    "2. **Zero iff equal:** $D_{KL}(p \\| q) = 0$ if and only if $p = q$\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "2. Perform eigendecomposition (connecting back to your Linear Algebra notebook!)\n",
    "3. Identify the **principal component direction** — the direction of maximum variance\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "1. **Descriptive Statistics** — Mean, median, mode, variance, and standard deviation give us the tools to summarize datasets with a few key numbers.\n",
    "\n",
    "2. **Covariance and Correlation** — Covariance tells us if two features move together; correlation normalizes this to $[-1, 1]$. The covariance matrix captures all pairwise relationships and is central to PCA.\n",
    "\n",
    "3. **Maximum Likelihood Estimation** — MLE formalizes the intuitive idea of finding the parameters that make our data most probable. It connects directly to loss functions: MSE = MLE under Gaussian noise.\n",
    "\n",
    "4. **Information Theory** — Entropy measures uncertainty, cross-entropy measures prediction quality, and KL divergence measures the gap between two distributions. Cross-entropy loss = the standard classification loss in deep learning.\n",
    "\n",
